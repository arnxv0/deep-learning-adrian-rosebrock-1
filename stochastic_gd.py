...
# as a trainable parameter within thr weight matrix
X = np.c_[X, np.ones((X.shape[0]))]

# partition the data into training the testing splits using 50% of
# the data for training and the remaining 50% for testing.
(trainX, testX, trainY, testY) = train_test_split(X, y, test_size=0.5, random_state=69)

# initializing our weight matrix and list of losses
print("[INFO] training...")
...
for epoch in np.arange(0, args["epochs"]):
    ...
    for (batchX, batchY) in next_batch(X, y, args["batch_size"]):
        # take the product between our current batch of features
        # and the weight matrix, then pass the value through our
        # activate function
